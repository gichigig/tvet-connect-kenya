...
CREATE POLICY "Lecturers can manage their timetable" 
...
CREATE POLICY "Students can update their pending retakes" 
  ON public.retakes FOR UPDATE 
...
CREATE POLICY "Lecturers can view and approve retakes for their units" 
  ON public.retakes FOR SELECT 
  USING (
    unit_id IN (
      SELECT id FROM public.units WHERE lecturer_id = auth.uid()
    ) OR is_admin_user(auth.uid())
  );

CREATE POLICY "Lecturers can approve retakes for their units" 
  ON public.retakes FOR UPDATE 
  USING (
    unit_id IN (
      SELECT id FROM public.units WHERE lecturer_id = auth.uid()
