...
export const SemesterPlanCalendar = () => {
  const { user } = useAuth();
  const { activities, getActivitiesForDate } = useSemesterPlan();
  const [selectedDate, setSelectedDate] = useState<Date>(new Date());
  const [calendarActivities, setCalendarActivities] = useState<ActivityItem[]>([]);
  ...
  // Convert SemesterPlanActivity to ActivityItem format
  useEffect(() => {
    if (!user) return;
    ...
    // Convert SemesterPlanActivity to ActivityItem format
    const convertedActivities: ActivityItem[] = activities.map(activity => ({
      id: activity.id,
    ...
    setCalendarActivities(convertedActivities);
    setIsLoading(false);
  }, [user, activities]);
  ...
  }, [calendarActivities, selectedDate]);
