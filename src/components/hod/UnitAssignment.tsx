...
export const UnitAssignment = () => {
  ...
  const lecturers = allUsers.filter(u => u.role === 'lecturer' && u.departmentId === user?.departmentId);
  const departmentUnits = units.filter(unit => 
    unit.course === user?.departmentId && unit.id // Filter by available units
  );
          ...
          <Card key={unit.id}>
            ...
            <CardContent className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center text-sm">
                    ...
                    <span className="font-medium">
                      {allUsers.find(u => u.id === unit.lecturerId)?.firstName} {allUsers.find(u => u.id === unit.lecturerId)?.lastName}
                    </span>
                  </div>
              ...
              <div className="text-xs text-gray-500 pt-2 border-t">
                {unit.credits} credits Ã¢â‚¬Â¢ {unit.enrolled}/{unit.capacity} students
              </div>
            </CardContent>
          </Card>
        ))}
