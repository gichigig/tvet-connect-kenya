...
const TimetableManagement = () => {
  ...
  const handleUploadTimetable = async () => {
    ...
    try {
      // Upload file to S3
      const fileUrl = await uploadStudentDocument(uploadForm.file, `timetable_${Date.now()}`);
  ...
  const handleDistributeTimetable = async () => {
    ...
    try {
      ...
      if (distributionSettings.targetLecturers) {
        targetLecturers = lecturers.filter(lecturer => {
          if (distributionSettings.specificCourse) {
            // Check if lecturer teaches units for this course
            const lecturerUnits = units.filter(unit => unit.lecturerName === `${lecturer.firstName} ${lecturer.lastName}`);
        ...
        for (const student of targetStudents) {
          notificationPromises.push(
            createNotification({
              ...
              type: 'announcement',
              actionUrl: selectedUploadedTimetable.fileUrl,
              data: { 
      ...
      const updatedTimetable = {
        ...
        distributionStatus: {
          emailSent: distributionSettings.sendEmail,
          notificationSent: distributionSettings.sendNotification,
          studentsNotified: targetStudents.length,
          lecturersNotified: targetLecturers.length
        }
      };
