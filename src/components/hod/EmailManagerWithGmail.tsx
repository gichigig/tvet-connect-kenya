      ...
      <Dialog open={isEmailDialogOpen} onOpenChange={setIsEmailDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              From: {selectedEmail?.from} • To: {selectedEmail?.to} • {selectedEmail ? formatDate('receivedDate' in selectedEmail ? selectedEmail.receivedDate : selectedEmail.date) : ''}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="flex gap-2">
              {selectedEmail && getStatusBadge('status' in selectedEmail ? selectedEmail.status : (selectedEmail.isRead ? 'read' : 'unread'))}
              {selectedEmail && getPriorityBadge('priority' in selectedEmail ? selectedEmail.priority : 'medium')}
