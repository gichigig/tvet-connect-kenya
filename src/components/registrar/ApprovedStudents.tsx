...
export const ApprovedStudents = () => {
  ...
  const { students: approvedStudents } = useStudents();
  const { courses } = useCoursesContext();
  const { toast } = useToast();

  // Mock functions for missing context methods
  ...
  // Get unique values for filters from all students with safe property access
  const availableCourses = [...new Set(approvedStudents.map(s => (s as any).courseName || s.course).filter(Boolean))];
  const availableYears = [...new Set(approvedStudents.map(s => (s as any).academicYear).filter(Boolean))];
  const availableSemesters = ['semester_1', 'semester_2', 'semester_3'];

  const filteredStudents = approvedStudents.filter(student => {
    const extStudent = student as any; // Type assertion for extra properties
    const matchesSearch = student.firstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         student.lastName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         student.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         student.admissionNumber?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         extStudent.nationalId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         extStudent.phone?.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesCourse = selectedCourse === "all" || 
                         extStudent.courseName === selectedCourse ||
                         student.course === selectedCourse;
    const matchesYear = selectedYear === "all" || extStudent.academicYear === selectedYear;
    const matchesSemester = selectedSemester === "all" || String(student.semester) === selectedSemester;
  ...
  const exportStudentData = () => {
    const csvData = filteredStudents.map(student => {
      ...
      return {
        ...
        'Course': extStudent.courseName,
        'Academic Year': extStudent.academicYear,
        'Semester': student.semester,
        'National ID': extStudent.nationalId,
        'Date of Birth': extStudent.dateOfBirth,
        'Gender': extStudent.gender,
        'Guardian Name': extStudent.guardianName,
        'Guardian Phone': extStudent.guardianPhone
      };
    });
                      ...
                      <TableRow key={student.id}>
                        <TableCell>
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{(student as any).nationalId}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="space-y-1">
                            <div className="text-sm">{student.email}</div>
                            <div className="text-sm text-muted-foreground">{(student as any).phone || (student as any).guardianPhone || 'N/A'}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium text-sm">
                              {(student as any).courseName || courses.find((course) => course.id === student.course)?.name || student.course || 'N/A'}
                            </div>
                        ...
                        <TableCell>
                          <div className="space-y-1">
                            <div className="text-sm">{(student as any).academicYear}</div>
                            <div className="text-xs text-muted-foreground">
                              Year {(student as any).academicYear}
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                  <div className="space-y-4">
                                    <div>
                                      ...
                                      <div className="space-y-2 text-sm">
                                        ...
                                        <div><strong>Phone:</strong> {(student as any).phone || (student as any).guardianPhone || 'N/A'}</div>
                                        <div><strong>National ID:</strong> {(student as any).nationalId}</div>
                                        <div><strong>Date of Birth:</strong> {(student as any).dateOfBirth || 'N/A'}</div>
                                        <div><strong>Gender:</strong> {(student as any).gender || 'N/A'}</div>
                                      </div>
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-2 text-sm">
                                        <div><strong>County:</strong> {(student as any).county || 'N/A'}</div>
                                        <div><strong>Sub-County:</strong> {(student as any).subcounty || 'N/A'}</div>
                                        <div><strong>Ward:</strong> {(student as any).ward || 'N/A'}</div>
                                        <div><strong>Postal Address:</strong> {(student as any).postalAddress || 'N/A'}</div>
                                        <div><strong>Postal Code:</strong> {(student as any).postalCode || 'N/A'}</div>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div className="space-y-4">
                                    <div>
                                      ...
                                      <div className="space-y-2 text-sm">
                                        <div><strong>Name:</strong> {(student as any).guardianName || 'N/A'}</div>
                                        <div><strong>Phone:</strong> {(student as any).guardianPhone || 'N/A'}</div>
                                        <div><strong>Email:</strong> {(student as any).guardianEmail || 'N/A'}</div>
                                        <div><strong>Relationship:</strong> {(student as any).guardianRelationship || 'N/A'}</div>
                                        <div><strong>Address:</strong> {(student as any).guardianAddress || 'N/A'}</div>
                                      </div>
                                    </div>
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-2 text-sm">
                                        <div><strong>Course:</strong> {(student as any).courseName || courses.find((course) => course.id === student.course)?.name || student.course || 'N/A'}</div>
                                        <div><strong>Academic Year:</strong> {(student as any).academicYear}</div>
                                        ...
                                        <div><strong>Account Status:</strong> 
                                          <Badge variant={(student as any).accountActive ? "default" : "destructive"} className="ml-2">
                                            {(student as any).accountActive ? "Active" : "Inactive"}
                                          </Badge>
                                        </div>
                                        <div><strong>Previous Education:</strong> {(student as any).previousEducation || 'N/A'}</div>
                                        <div><strong>Previous Grade:</strong> {(student as any).previousGrade || 'N/A'}</div>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{(student as any).phone || (student as any).guardianPhone || 'N/A'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>{(student as any).academicYear}</span>
                  </div>
