...
export const CourseContainer = ({ course, onCourseUpdate }: CourseContainerProps) => {
  const { addCreatedUnit, updateCreatedUnit, deleteCreatedUnit, createdUnits } = useUnits();
  ...
  const lecturers = getAllUsers().filter(user => user.role === 'lecturer' && user.approved);
  ...
  const handleUpdateUnit = async () => {
    ...
    try {
      ...
      await updateCreatedUnit(selectedUnit.id, unitData);
  ...
  const handleDeleteCourse = async () => {
    ...
    try {
      ...
      const deleteUnitPromises = courseUnits.map(unit => deleteCreatedUnit(unit.id));
