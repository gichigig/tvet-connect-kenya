...
export const CourseManagement = () => {
  ...
  // Load departments and institutions from database
  ...
  // Load departments and institutions when user changes
  useEffect(() => {
    if (user?.id) {
      loadDepartments();
      loadInstitutions();
  ...
  useEffect(() => {
    const coursesFromUnits = units.reduce((acc, unit) => {
      ...
      if (existingCourse) {
        // existingCourse.units.push(unit); // Removed to avoid type conflicts
        ...
        acc.push({
          ...
          department: 'General', // Default since Unit doesn't have department
          duration: Math.max(...units.filter(u => u.course === unit.course).map(u => u.year)),
          totalCredits: unit.credits,
          units: [], // Simplified - remove units array to avoid type conflicts
  ...
  // Get unique departments from database instead of courses
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => setNewCourse(prev => ({ ...prev, department: value }))}
