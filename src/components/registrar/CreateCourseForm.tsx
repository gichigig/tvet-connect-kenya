...
export const CreateCourseForm: React.FC = () => {
  ...
  const loadDepartments = async () => {
    ...
    try {
      ...
      setDepartments(data);
      console.log('📊 Loaded departments for course creation:', data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const courseData = {
        ...
        credits: formData.duration * 30, // Estimate 30 credits per year
        prerequisites: formData.requirements.filter(req => req.trim() !== '')
      };
