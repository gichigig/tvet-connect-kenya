...
import {
  ...
  rejectRetakeRequest
} from "@/integrations/supabase/retakeRequests";
...
export const RetakeManager = () => {
  ...
  const handleApproveRequest = async (requestId: string, studentName: string) => {
    // For approval, we need exam details - this is a simplified version
    const examDetails = {
      exam_date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Default to next week
      exam_time: "09:00",
      venue: "Exam Hall A"
    };
    await approveRetakeRequest(requestId, examDetails, "admin", reviewNotes);
    toast({
  ...
  const handleRejectRequest = async (requestId: string, studentName: string) => {
    await rejectRetakeRequest(requestId, "admin", reviewNotes);
    toast({
  ...
  const handleViewRequest = (request: RetakeRequest) => {
    ...
    setReviewNotes(request.admin_comments || "");
  ...
  const filteredRequests = retakeRequests.filter(request => {
    const matchesSearch = request.student_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         request.unit_code.toLowerCase().includes(searchTerm.toLowerCase());
                ...
                <TableRow key={request.id}>
                  <TableCell>
                    <div>
                      ...
                      <div className="text-sm text-gray-500">ID: {request.student_id}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div>
                      <div className="font-medium">{request.unit_code}</div>
                      <div className="text-sm text-gray-500">{request.unit_name}</div>
                  ...
                  <TableCell>
                    <Badge variant="outline">{request.exam_type}</Badge>
                  </TableCell>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{selectedRequest.student_name}</p>
                  <p className="text-sm text-gray-500">ID: {selectedRequest.student_id}</p>
                </div>
                <div>
                  <h4 className="font-medium text-sm text-gray-600">Unit</h4>
                  <p>{selectedRequest.unit_code} - {selectedRequest.unit_name}</p>
                  <p className="text-sm text-gray-500">
                    {selectedRequest.exam_type} exam
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <Badge variant="outline">{selectedRequest.exam_type}</Badge>
              </div>
              ...
              {selectedRequest.status === 'pending' && (
                <div className="flex justify-end space-x-2">
                  ...
                  <Button
                    variant="destructive"
                    onClick={() => handleRejectRequest(selectedRequest.id, selectedRequest.student_name)}
                  >
                    <XCircle className="w-4 h-4 mr-1" />
                    Reject
                  </Button>
                  <Button
                    onClick={() => handleApproveRequest(selectedRequest.id, selectedRequest.student_name)}
              ...
              {selectedRequest.status !== 'pending' && selectedRequest.admin_comments && (
                <div>
                  <h4 className="font-medium text-sm text-gray-600">Previous Review Notes</h4>
                  <p className="text-sm bg-gray-50 p-3 rounded">{selectedRequest.admin_comments}</p>
                </div>
              )}
            </div>
          )}
